...
@mark.parametrize("lat,region,surfaces,dz,neighbors", [
    param(1, '-1', '1  rpp  -1 1  -1.5 1.5  -3 7', 10.,
          [(2., 0.), (0., 3.), (-2., -3.)], id='rect'),
    # Listing the bottom plane first puts index k below the element
    param(1, '-11 12 -13 14 16 -15', _LAYER_PLANES, -10.,
          [(2., 0.), (0., 3.), (-2., -3.)], id='rect-bottom-first'),
    param(2, '-1', '1  rhp  0 0 -3   0 0 10   1 0 0', 10.,
          [(2., 0.), (1., SQRT3), (-1., SQRT3)], id='hex'),
])
def test_single_layer_lattice(lat, region, surfaces, dz, neighbors, k):
